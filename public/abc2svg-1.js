...
abc2svg.b40k=new Int8Array([36,1,2,3,8,2,2,7,8,13,14,2,8,13,14,19,20,13,14,19,20,25,2,19,24,25,30,31,2,25,30,31,36,37,2,31,36,37,42,43])
...
switch(dd.glyph){case"brth":case"caes":case"lphr":case"mphr":case"sphr":case"short":case"tick":y=staff_tb[s.st].topbar+2+dd.hd
...
dd.ty=ty}
return dd}
function deco_cnv(s,prev){var i,j,dd,nm,note,s1,court
...
") format("truetype")'
var font_scale_tb={serif:1,serifBold:1,'sans-serif':1,'sans-serifBold':1,Palatino:1.1,monospace:1.35},txt_ff="text,serif",fmt_lock={}
...
continue
switch(k){case"fid":case"used":break
...
break
case C.REST:if(s.rep_nb>1||s.bar_mrep){s.y=12
s.ymx=38
s.ymn=0
break}
if(voice_tb.length==1){s.y=12;s.ymx=24;s.ymn=0
break}
...
s.seqst=true
for(s=last_s;s.ts_next&&!s.ts_next.seqst;s=s.ts_next);if(s.ts_next&&s.ts_next.type!=C.CLEF&&!s.ts_next.a_ly)
...
break}
multicol={posy:posy,maxy:posy,lm:cfmt.leftmargin,rm:cfmt.rightmargin,w:cfmt.pagewidth,sc:cfmt.scale}
break
...
img.chg=1
break
case"mc_end":if(!multicol){error(1,s,"%%multicol end without start")
break}
if(posy<multicol.maxy)
posy=multicol.maxy
cfmt.leftmargin=multicol.lm
...
return
return"||||||||||||||||".slice(0,n)}
function new_block(subtype){var c_v,s={type:C.BLOCK,subtype:subtype,dur:0}
c_v=curvoice
...
break}
s={type:C.PART,text:text,time:tim}
if(!new_ctrl(s))
break
sym_link(s)
...
if(curvoice.tr_sco&&!note.notrp){i=nt_trans(note,i)
...
var deco_str_style={crdc:{dx:0,dy:5,style:'font:italic 14px text,serif',anchor:' text-anchor="middle"'},dacs:{dx:0,dy:3,style:'font:16px text,serif',anchor:' text-anchor="middle"'},pf:{dx:0,dy:5,style:'font:italic bold 16px text,serif'},'@':{dx:0,dy:5,style:'font:12px text,sans-serif'}}
...
break}}
if(!fl){while(prev.type==C.GRACE&&vtb[prev.v]&&!vtb[prev.v].bar_type){vtb[prev.v]=prev
prev=prev.ts_prev
fl=1}}
if(v==undefined)
return
...
map[0]=a[i].split(',')
break
case"print=":case"play=":case"print_notrp=":if(!a[++i]){syntax(1,not_enough_p)
break}
...
map[2]=a[i]
break}}}
function new_ctrl(s){var a,ty=(s.type+curvoice.time).toString()
if(!parse.ctrl)
parse.ctrl={}
a=parse.ctrl[ty]
if(a){if(a&1<<curvoice.v)
return 1
...
tsfirst=null}}
function key_trans(){var i,n,a_acc,b40,s=curvoice.ckey,ti=s.time||0
...
b40=(s.k_b40+200+n)%40
i=abc2svg.b40k[b40]-b40
if(i){curvoice.tr_sco+=i
b40+=i}
s.orig=clone(s)
s.k_b40=b40
if(!s.k_none)
...
par_sy.voices=[]}else{voice_adj(true)
...
if(s.prev)
s.clef_small=1}
function get_key(parm){var v,p_voice,transp,sndtran,nt,a=new_key(parm),s_key=a[0],s=s_key,empty=s.k_sf==undefined&&!s.k_a_acc
a=a[1]
if(empty)
s.invis=1
...
a=[]}
if(!glovar.ulen)
glovar.ulen=C.BLEN/8;goto_tune()}else if(!empty){s.k_old_sf=curvoice.ckey.k_sf
curvoice.ckey=s
sym_link(s)
if(curvoice.tr_sco)
curvoice.tr_sco=0}
...
function gch_tr1(p,tr){var i,o,n,a,ip,b40,csa=p.split('/')
...
if(w&&x+w>realwidth)
x=realwidth-w
if(an.pos==C.SL_ABOVE){y=y_get(s.st,true,x,w)+2
...
abc2svg.version="v1.22.9-23-02f440d127";abc2svg.vdate="2023-08-30"