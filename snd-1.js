...
function AbcPlay(i_conf){var conf=i_conf,init={},audio=ToAudio(),audio5,midi5,current,abcplay={clear:audio.clear,add:audio.add,set_sfu:function(v){if(v==undefined)
    ...
    s.type=C.REST
    ch=[0]
    r=0}else{switch(a[2]){case"#":case"â™¯":r++;break
    case"b":case"â™­":r--;break}
    ...
    b=abc2svg.letmid[b]
    if(b!=undefined){switch(a[4][1]){case"#":case"â™¯":b++;if(b>=12)b=0;break
    case"b":case"â™­":b--;if(b<0)b=11;break}}}}
    if(b==undefined)
    b=0
    ch=chcr(b,ch)
    n=ch.length
